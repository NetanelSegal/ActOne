...
import { createScriptBodySchema, updateScriptBodySchema } from "../../shared/schemas/api.js";
...
router.get("/", async (req: Request, res: Response) => {
  ...
  res.json(scripts.map((s) => ({ id: s.id, user_id: s.user_id, title: s.title, content: s.content, created_at: s.created_at })));
});
...
router.get("/:id", async (req: Request, res: Response) => {
  ...
  res.json({ id: script.id, user_id: script.user_id, title: script.title, content: script.content, created_at: script.created_at });
});
...
router.post("/", async (req: Request, res: Response) => {
  ...
  res.status(201).json({ id: script.id, user_id: script.user_id, title: script.title, content: script.content, created_at: script.created_at });
});
